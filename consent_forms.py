...
CLINIC_LOGO  = os.path.join(os.path.dirname(__file__), "pet_wellness_logo.png")  # optional
...
class ConsentFormsScreen(QWidget):
    ...
    def _populate_body_from_template(self, force: bool = False):
        ...
        conn.execute("PRAGMA foreign_keys=ON;"); cur = conn.cursor()
        cur.execute("SELECT name, body_text FROM consent_templates WHERE template_id=?",(tid,))
        row = cur.fetchone()
        conn.close()
        if not row:
            return
    ...
    def load_forms(self):
        ...
        if term:
            rows = [r for r in rows if term in (r[1] or "").lower() or term in (r[2] or "").lower()]
    ...
    def _on_select(self):
        ...
        row = cur.fetchone(); conn.close()
        if not row: return
        (pid, pname, ftype, body, signed_by, relation, status, fup, sigpath, tpl_id) = row
    ...
    def _draw_header(self, pdf, W, H):
        ...
        if CLINIC_LOGO and os.path.exists(CLINIC_LOGO):
            try:
                ...
                pdf.drawImage(img, 50, y-20, width=80, height=40, preserveAspectRatio=True, mask='auto')
            except Exception:
                pass
        # Text (right)
        ...
        y -= 16
        if CLINIC_ADDR1: pdf.drawRightString(W-50, y, CLINIC_ADDR1); y -= 14
        if CLINIC_ADDR2: pdf.drawRightString(W-50, y, CLINIC_ADDR2); y -= 14
    ...
    def on_export_pdf(self):
        ...
        try:
            ...
            if os.path.exists(logo_path):
                ...
                pdf.drawImage(logo, W - 140, H - 80, width=90, height=60, preserveAspectRatio=True, mask='auto')
            ...
            pdf.drawString(50, y, f"Signed By: {signed_by or ''}    Relation: {relation or ''}");
            y -= 40
