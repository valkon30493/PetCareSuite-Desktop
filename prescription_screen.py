...
class PrescriptionScreen(QWidget):
    def __init__(self, all_patients):
        ...
        self.qty_in.setMinimum(1)
        self.start_in = QDateEdit(QDate.currentDate())
        self.start_in.setCalendarPopup(True)
        self.end_in = QDateEdit(QDate.currentDate())
    ...
    def on_export(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Save CSV", "prescriptions.csv", "*.csv"
        )
        if not path:
            return
