# reports.py
import os, sqlite3
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton,QDateEdit,
                               QTableWidget, QTableWidgetItem, QHeaderView, QSpinBox, QFileDialog, QMessageBox)
...
def get_z_report_data(business_date: str, cutoff_hour: int = 0):
    ...
    rows = cur.fetchall()
    by_rate = [{"rate": int(r[0]), "net": float(r[1] or 0.0),
                "vat": float(r[2] or 0.0), "gross": float(r[3] or 0.0)} for r in rows]
    totals = {
    ...
    tenders = [{"method": m or "Other", "payments_total": float(p or 0.0), "refunds_total": float(r or 0.0)}
    ...
    return {
        "window":  {"start_ts": start_ts, "end_ts": end_ts, "cutoff_hour": int(cutoff_hour)},
        "sales":   {
...
def export_z_report_pdf(parent, business_date: str, cutoff_hour: int = 0):
    ...
    path, _ = QFileDialog.getSaveFileName(parent, "Save Z-Report (PDF)", default_name, "PDF Files (*.pdf)")
    if not path:
        return
    ...
    H2    = ParagraphStyle('H2', parent=styles['Heading2'], fontSize=13, leading=15, spaceAfter=6)
    ...
    order = ["CASH","CARD","BANK_TRANSFER","ONLINE","CHEQUE","GIFT_CARD","STORE_CREDIT"]
    by_name = {t["method"]: t for t in tend["by_method"]}
    for name in order:
        row = by_name.get(name, {"payments_total":0.0, "refunds_total":0.0})
        tender_rows.append([
            name,
            f"€{row['payments_total']:.2f}",
            f"€{row['refunds_total']:.2f}"
        ])
    # Net total line
    ...
    sales_lines = [
        ["Invoices count (FINAL/POSTED/PAID):",  str(int(sales.get("invoice_count", 0)))],
...
def export_z_report_excel(parent, business_date: str, cutoff_hour: int = 0):
    ...
    data   = get_z_report_data(business_date, cutoff_hour)
    sales  = data["sales"]; tend = data["tenders"]; win = data["window"]
    ...
    wb = Workbook(); ws = wb.active; ws.title = "Z_REPORT"
    B = Font(bold=True); R = Alignment(horizontal="right"); C = Alignment(horizontal="center")
    ...
    ws.cell(r,1,"Business Date"); ws.cell(r,2,business_date); r += 1
    ws.cell(r,1,"Cutoff Hour (0–23)"); ws.cell(r,2,cutoff_hour); r += 1
    ws.cell(r,1,"Start Timestamp"); ws.cell(r,2,win["start_ts"]); r += 1
    ws.cell(r,1,"End Timestamp");   ws.cell(r,2,win["end_ts"]);   r += 2
    ...
    # SALES section
    ws.cell(r,1,"SALES (Invoices by invoice_date)").font = B; r += 2
    ws.cell(r,1,"Invoices count (FINAL/POSTED/PAID)"); ws.cell(r,2,int(sales["invoice_count"])); r += 1
    ws.cell(r,1,"Gross sales");                        ws.cell(r,2,float(sales["totals"]["gross"])).alignment = R; r += 1
    ws.cell(r,1,"VAT / TAX BREAKDOWN (FINAL & POSTED; invoice_date in window)").font = B; r += 1
    ws.cell(r,1,"Tax collected");                      ws.cell(r,2,float(sales["totals"]["vat"])).alignment = R; r += 1

    # VAT breakdown table (Rate/Net/VAT/Gross)
    ws.cell(r,1,"Rate").font=B; ws.cell(r,2,"Net taxable").font=B
    ws.cell(r,3,"VAT amount").font=B; ws.cell(r,4,"Gross (Net+VAT)").font=B; r += 1
    ...
    ws.cell(r,2,sales["totals"]["net"]).alignment=R; ws.cell(r,2).font=B
    ws.cell(r,3,sales["totals"]["vat"]).alignment=R; ws.cell(r,3).font=B
    ws.cell(r,4,sales["totals"]["gross"]).alignment=R; ws.cell(r,4).font=B
    r += 2

    # Cash drawer section (placeholders; zero until you add drawer events/refunds tables)
    zeros = ["Opening float (sum of sessions starting in window)",
             "Cash in (drawer events)","Cash out + payouts (drawer events)",
             "Net cash tenders (payments - refunds)","Expected closing cash",
             "Counted cash (sum of sessions ending in window)","Over / Short (Counted - Expected)"]
    vals  = [0,0,0,tend["net_total"],0,0,0]
    for label, val in zip(zeros, vals):
        ws.cell(r,1,label); ws.cell(r,2,float(val)).alignment=R; r += 1
    ...
    path, _ = QFileDialog.getSaveFileName(parent, "Save Z-Report (Excel)", default, "Excel Files (*.xlsx)")
    if not path: return
    wb.save(path)
    QMessageBox.information(parent, "Z-Report", f"Saved: {path}")
...
class ZReportWidget(QWidget):
    def __init__(self):
        ...
        self.vat_tbl.setHorizontalHeaderLabels(["VAT %", "Net excl. VAT", "VAT", "Gross incl. VAT"])
        self.vat_tbl.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        ...
        self.pay_tbl.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
    ...
    def refresh(self):
        ...
        for r in by_rate:
            ...
            self.vat_tbl.setItem(row, 1, QTableWidgetItem(f"{float(r.get('net', 0.0)):.2f}"))
            self.vat_tbl.setItem(row, 2, QTableWidgetItem(f"{float(r.get('vat', 0.0)):.2f}"))
            self.vat_tbl.setItem(row, 3, QTableWidgetItem(f"{float(r.get('gross', 0.0)):.2f}"))

        # â â  Payments by method table (payments & refunds columns)
        # ensure headers match the new structure
        self.pay_tbl.setColumnCount(3)
        self.pay_tbl.setHorizontalHeaderLabels(["Method", "Payments total", "Refunds total"])
        self.pay_tbl.setRowCount(0)

        # show common methods in a consistent order
        order = ["CASH", "CARD", "BANK_TRANSFER", "ONLINE", "CHEQUE", "GIFT_CARD", "STORE_CREDIT"]
        by_name = {m.get("method"): "{:.2f}|{:.2f}".format(
            float(m.get("payments_total", 0.0)),
            float(m.get("refunds_total", 0.0))
        ) for m in by_method}
