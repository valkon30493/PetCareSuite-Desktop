...
class ReportsAnalyticsScreen(QWidget):
    ...
    def export_revenue_pdf(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save PDF", "revenue_report.pdf", "PDF Files (*.pdf)"
        )
        if not path:
            return
    ...
    def unpaid_invoices_tab(self):
        ...
        self.unpaid_table.setHorizontalHeaderLabels(
            ["Invoice ID", "Appointment ID", "Patient", "Amount Due", "Created At"]
        )
    ...
    def export_unpaid_csv(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Save CSV", "unpaid_invoices.csv", "CSV Files (*.csv)"
        )
        if not path:
            return
        ...
        with open(path, "w", newline="", encoding="utf-8") as f:
            ...
            for r in range(self.unpaid_table.rowCount()):
                row = [
                    self.unpaid_table.item(r, c).text()
                    for c in range(self.unpaid_table.columnCount())
                ]
                writer.writerow(row)
    ...
    def export_species_pdf(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save PDF", "species_report.pdf", "PDF Files (*.pdf)"
        )
        if not path:
            return
    ...
    def export_top_items_pdf(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save PDF", "top_items_report.pdf", "PDF Files (*.pdf)"
        )
        if not path:
            return
    ...
    def export_busiest_days_pdf(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save PDF", "busiest_days.pdf", "PDF Files (*.pdf)"
        )
        if not path:
            return
    ...
    def export_vet_pdf(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save PDF", "appointments_by_vet.pdf", "PDF Files (*.pdf)"
        )
        if not path:
            return
